...
### 3. 머신러닝의 기본 요소
- 하나의 데이터는 $n$ 차원의 열벡터 $x = [x_1, x_2, ..., x_n]^T$ 로 표현하며, 데이터 처리는 벡터 연산으로 정의함
...
- 목적 함수 : 주어진 데이터 집합을 이용하여 학습 시스템이 달성하고자 하는 목표를 기계가 알 수 있는 함수로 정의한 것
- 오차 함수 : 학습 시스템의 출력과 원하는 출력의 차이(오차)로 정의된 목적 함수
- 성능 평가 기준 : 학습 오차, 테스트 오차, 일반화 오차
- 교차검증법 : 제
