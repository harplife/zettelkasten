...
- **확률 (Probability)** : 어떤 사건이 일어날 가능성을 0과 1 사이의 실수로 표시한 것..이라 간단하게 표현할 수 있지만, 실제로 확률의 "정의"라는 것은 아주 심오한 토픽이다.
    - 사람은 일반적으로 지식, 경험, 통찰력, 정보에 입각하여 어떤 사건이 발생할 확률을 계산하는데, 이를 **주관적(Subjective) 확률**이라 볼 수 있다.
    - 반대로, 실제 실험을 통해 수학적으로 특정 사건의 발생 확률을 계산하는 것을 **객관적(Objective) 확률**이라 볼 수 있다.
    - 객관적 확률에는 두 가지 추정 방안이 있는데, 이는 [빈도주의적 추정 (Frequentist Inference)](https://en.wikipedia.org/wiki/Frequentist_inference)과 [베이지안 추정 (Bayesian Inference)](https://en.wikipedia.org/wiki/Bayesian_inference) 이다.
