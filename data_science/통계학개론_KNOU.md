...
- **사분위수 (Quantiles)** : 데이터를 크기 순서대로 정렬 했을 때 이 데이터를 4등분하는 3개의 값.
...
- 백분위수 (Percentile) : 교과서 정의는 "전체 데이터의 p%가 이 값보다 작거나 같은 값"이다. 데이터를 크기 순서대로 정렬 했을 때 이 데이터를 100등분하는 99개의 값으로 이해하는게 좋을 듯 하다.
