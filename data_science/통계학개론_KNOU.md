...
- **이산형 표본공간 (Discrete Sample Space)** : 표본공간의 원소 개수가 유한개(Finite)거나, 무한하나 셀 수 있는 경우(Countably Infinite)를 뜻한다.
    - 주사위 던지기 같은 경우, 총 6가지 결과가 나올 수 있으니 원소 개수가 유한하다. 따라서, 주사위 던지기의 표본공간은 이산형이다.
    - 피자 배달이 20분 내에 도착한다는 조건 하에, 도착 시간이 0분~20분 사이에 있기 때문에 무한하되 ([실수는 셀 수 없다](https://blog.naver.com/minzzang68/221948570645)) 도착 시간 범위는 셀 수 있기 때문에 (0~5분, 5~10분) 원소 개수가 셀 수 있는 무한이다. 따라서, 피자 배달 시간 문제의 표본공간은 이산형이다.
