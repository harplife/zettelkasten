...
조건명제의 특징은 전건이 참이면 후건이 참이된다는 것을 "보장(Guarantee)"한다는 것이다. 조건명제의 진리값은 "이 보장이 지켜졌는가?"의 답으로 볼 수 있다. 따라서,
1. 전건이 참이고 후건이 참이면 보장을 지키는 것이기 때문에 조건명제는 참이된다.
2. 전건이 참이고 후건이 거짓이면 보장을 지켜지지 않았기 때문에 조건명제는 거짓이 된다.
3. 전건이 거짓인 경우에 대해서 보장을 하지 않았기 때문에, 보장은 지켜지지 않았기 않았기 (NOT NOT) 때문에 조건명제는 참이된다.
...
조건명제 진리표에 3번째와 4번째를 보면 좀 헷갈릴 수 도 있다. 전건이 부정인 경우, 왜 후건의 진리값과 상관없이 연산 결과는 
