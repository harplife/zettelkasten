...
### 조건명제
**조건명제(Conditional Proposition)** : 명제 `𝑃`와 `𝑄`가 있을 때, 명제 `𝑃`로 인해 명제 `𝑄`가 영향을 받는 경우, `𝑃`와 `𝑄`의 합정명제를 조건명제라 한다. 논리연산자로 두 명제 사이에 `→`, `⇒`, 또는 `⊃`로 표현된다 (예: `𝑃 → 𝑄`).

조건명제의 `𝑃`는 **전건(Antecedent)** 이라 불리운다.

조건명제의 `𝑄`는 **후건(Consequent)** 이라 불리운다.

논리를 영어로 이해하기도 어려운데, 한국어로 이해하려니 더 어렵다!! 

두 명제의 관계로부터 어떤 
명제에 숨어있는 뜻이 암시된다는 의미에서  **함의/함축(Implication)** 라고 불리기도 한다.
