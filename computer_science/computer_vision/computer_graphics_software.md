...
### 기하구조 셰이더
[기하구조 셰이더(Geometry Shader)](https://www.khronos.org/opengl/wiki/Geometry_Shader)는 하나 또는 서로 인접(Adjacent)한 여러 개의 기본 도형을 입력받아 하나 이상의 새로운 기본 도형을 출력하거나 또는 아예 출력하지 않는다.

기하구조 셰이더는 정점 셰이더(또는 테셀레이션 셰이더)와 정점 후처리 사이에 선택적으로 적용될 수 있다.

기하구조 셰이더의 강점은 여러 도형을 하나의 도형으로 처리할 수 있다는 것이다. 이 뜻은,
1. 여러 도형 대신 한 개로 처리함으로 연산량과 메모리 사용량을 줄인다\*.
2. 여러 도형에 대하여 어떠한 변화를 줄 때 각 도형에 변화를 주는 대신 모든 도형에 대하여 한꺼번에 변화를 준다.

정점 3개로 이루어진 삼각형이 서로 인접하게 3개가 모인 영역이 있다면, 이 영역에 사용되는 정점은 총 9개이다. 기하구조 셰이더로 이 3개 삼각형을 한 개의 도형으로 처리하면, 반복되는 정점이 제거되어 이 영역에 사용되는 정점이 총 5개로 줄어든다. 이렇게 연결된 삼각형들로 이루어진 도형을 삼각형 스트립 (Triangle Strip)이라고 한다.

![[triangle_list_to_strip.png]]

여러 도형을 하나로 합치는 것 외에, 하나의 도형을 변형시키는 것도 가능한테, 주로 도형의 법선(Normal)을 그리고 싶을 때 이런 기능을 사용한다.

참고 : 모델에 조명이 이상하게 반사되는 영역이 보인다면, 전체 또는 해당 부분에 속한 다각형들에 법선을 그려봄으로 문제를 자세히 파악해볼 수 있다.

![[normal_on_triangle.jpg]]
