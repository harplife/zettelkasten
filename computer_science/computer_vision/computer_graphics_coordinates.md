...
편의를 위해서 이 면들을 시야각(View)으로 표현하겠다.

다시점 투영을 기준으로 정사 투영은 기본 시야각과 보조 시야각으로 나뉜다.

#### 기본 시야각
기본 시야각(Primary Views)은 각 시야각이 투영면과 수직이 되어 투영됨을 뜻한다.

이 시야각들을 나열하여 3D 물체를 표현할 수 있으며, 나열하는 순서에 따라 제1각법(First-angle) 또는 제3각법(Third-angle)으로 나뉜다.

![[multiview_projection_1st_and_3rd_angle_comparison.png]]

다시점 투영의 특징은 특정 3가지 시야각으로 3D 물체에 대한 정보를 충분히 표현할 수 있다는 것이다. 이 3가지 시야각은 밑에와 같다.
1. 물체를 윗면에서 바라보는 설계도/배치도(Plan)
2. 물체를 옆면(앞/뒤/왼쪽/오른쪽)에서 보는 입면도(Elevation)
3. 물체의 내면을 바라보는 단면도(Section)

#### 보조 시야각
보조 시야각(Auxiliary Views)은  





다시점 투영에 있어 이라고 하며,  것을  라고 한다. 
