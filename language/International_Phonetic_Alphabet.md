...
#define A [Monophthong](https://en.wikipedia.org/wiki/Monophthong) (Pure Vowel) is a single vowel phoneme characterized by a relatively stable articulatory configuration throughout its duration.
...
Modern GA notations sometimes (or completely) replace these vowels :
- `/ɔɪ/` is sometimes replaced with `/oɪ/`
- `/ɑ/` is completely replaced with `/a/` in `/aɪ/` and `/aʊ/`
...
#define **Yod-Coalescence** is a form of *reciprocal assimilative palatalization* where an alveolar consonant `/t,d,s,z/` merges with a following palatal approximant `/j/` (the "yod") to produce post-alveolar sounds `/ʃ,ʒ,d͡ʒ,t͡ʃ/`.
...
- `/t/` + `/j/` = `/tʃ/` --> nature, actually, virtue, won't-you
- `/d/` + `/j/` = `/dʒ/` --> educate, residual, soldier, graduate, did-you
- `/s/` + `/j/` = `/ʃ/` --> issue, assume, pressure
- `/z/` + `/j/` = `/ʒ/` --> azure, pleasure, leisure
...
#define **Rhotacization** (R-Coloring) is a form of *anticipatory assimilation* where a vowel's quality is modified by an immediately following `/ɹ/` sound in the same syllable.
- #define Rhotacization causes the vowel and the consonant `/ɹ/` to blend into a single (simultaenous), cohesive vocalic sound, known as an [R-Colored Vowel](https://en.wikipedia.org/wiki/R-colored_vowel) (Rhotic Vowel).
