...
| **Ratio Test**                 | Especially useful for factorials, exponentials, powers, and products.                                | Compute $\displaystyle L=\lim_{n\to\infty}\left(\frac{\lvert a_{n+1}\rvert}{\lvert a_n\rvert}\right)$. | If $L<1$, the series converges absolutely. If $L>1$, the series diverges. If $L=1$, the test is inconclusive.                                                       |
