...
2. RGB 색 공간에서 원추 세포는 초록에 가장 반응이 크고, 다음은 빨강, 마지막으로 파랑이다. 이 정보가 중요한 이유는, RGB 색 공간에서 [상대적 밝기 (Relative Luminance)](https://en.wikipedia.org/wiki/Relative_luminance)를 추출할 때 이 정보를 기준으로 만들어진 공식을 사용하기 때문이다. 참고로, 현실 세계에서는 노랑-초록색에 가장 반응이 크다.
3. "자극값"은 어떤 단위의 값이라기 보다는 상대적 (Relative) 값이다.
...
5. 매칭되면 빨강, 초록, 파랑 조명의 강도가 기록된다. 이 강도가 삼색 자극값이다.
...
1. [조건 등색 (Metamerism)](https://en.wikipedia.org/wiki/Metamerism_(color)) : 광원, 관측자, 관측조건 차이에 따라 두 물체의 색이 같아 보이거나 달라 보이는 현상. 서로 다른 파장의 색들을 혼합하여 다른 하나의 색을 구현하는 [[#가산혼합]]의 기반이 된다. 위 그래프를 보면 실제로 RGB 값들이 겹치는 것을 볼 수 있다.
...
삼색 자극값은 좀 거창한 정의를 가지고 있지만, MCM 테스트를 이해하고 보면 삼색 자극값은 그냥 특정 색에 대한 원색 (RGB)의 비율로 볼 수 있다.

#todo 여기서부터는 삼색 자극값을 어떻게 2차원 그래프로 변형했는지에 대한 내용이 들어가야 됨.
