...
위에 명시했듯이, 빛은 보이는 빛과 보이지 않는 빛이 있으며, 우리가 일반적으로 접하는 색은 보이는 빛과 물체의 상호작용으로 발생된 가시광선이다. 보이지 않는 빛과 물체의 상호작용으로 발생되는 가시광선도 있다 (예: [형광(Fluorescence)](https://en.wikipedia.org/wiki/Fluorescence)은 자외선을 받으면 가시광선을 방출하는 물질의 특성이다).
