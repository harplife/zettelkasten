...
내가 정리하고자 하는 것이 어떤 학문과 일치하는지 모르겠다. 이 노트의 목표는 #computer_graphics 에 입문하기 위해 색에 대한 기초지식을 정리하되, "빛은 흡수/반사/투과되며 사용되는 수식은 이거다" 또는 "모니터는 RGB 모델을 사용한다"보다는 더 깊게 나아가고 싶다.

이 노트의 핵심은 빛-물체-사람과의 관계가 될 것이다. 빛이 특성, 물체의 특성, 사람의 특성에 관련해서 최대한 자세히 정리한다. 색 공간/모델에 대하여 어느정도 정리할 건데, 이유는 색을 수학적으로 어떻게 표현하는 가를 이해하기 위해서이다.
...
1756년도에 [토마스 멜빌(Thomas Melvill)](https://en.wikipedia.org/wiki/Thomas_Melvill)은 [알코올 램프](https://en.wikipedia.org/wiki/Alcohol_burner)로 여러 종류의 염(Salt)를 태우며 방출되는 빛을 연구하는 프리즘 실험을 했다.
...
참고 : 안타깝게도 멜빌의 발견은 그 당시에 많은 주목을 받지 못 했다.

#### 스펙트럼 선
멜빌은 스펙트럼에 보이는 검은 부분들이 무엇을 의미하는지 몰랐다. 더불어, 18세기에 사용되었던 프리즘과 렌즈는 스펙트럼을 자세히 연구하기에는 많이 부족했기 때문에 태양광 스펙트럼에도 "검은 선"이 있다는 것을 인지하지 못 했다.
...
더불어, 프라운호퍼는 태양과 다른 광원(예: 별, 금속, 염)의 스펙트럼을 비교하여 각 광원의 흑선들의 위치가 다르다는 것을 발견했고, 이 흑선들이 울러스턴이 말했던 "색의 경계선"이 아니라 빛을 발하는 광원의 특성이라는 이론을 제시하였다 - 이는 멜빌의 "원소의 특유 스펙트럼" 이론을 확정시켰다.
