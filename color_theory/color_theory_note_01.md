...
광자가 원자와 충돌하면, 원자는 들뜬 상태 (Excited State)로 변한다. 들뜬 원자는 다시 바닥 상태 (Ground State)로 안정화하기 위해 진동을 하며, 이 진동이 열 에너지와 또 다른 광자를 발생시킨다.

가시광선의 파장은 원자, 분자 (Molecule)보다 크기 때문에 상호작용을 하지 않는다. 따라서, 원자/분자는 색이 없다. 가시광선과 상호작용을 하기 위해서는, 분자들이 붙어있는 조직이어야 한다 - 분자 간의 거리와 분자들이 정리된 구조가 색에 영향을 준다.
...
여기서 "빛을 흡수한다"의 의미는 (일단 여러 자료를 취합해본 결과), 빛이 원자 (Atom)와 충돌하며 빛의 에너지를 원자로 이동시키며 원자를 들뜨게 (Excite)하고, 들뜬 원자는 진동을 한다는 것이다. 한 마디로, 흡수는 빛의 에너지가 원자의 진동으로 변환된다는 뜻이다.

원자는 진동하며 안정 상태로 돌아가려고 하며, 진동을 열 에너지로 변환하게 된다.

모든 물체도 마찬가지로, 빛을 흡수하고 열을 방출한다. 좀 더 다르게 표현하자면, 모

참고 : 여기서 명시하는 물체의 "입자"는, 분자 (Molecule)보다 큰 분자들의 조직이다. 분자는 빛의 파장보다 작기 때문에, 반사

하지만 물체는 완벽한 흑체가 아니기 때문에, 빛을 완전히 흡수하고 열 에너지로 변환하는게 아니다.
